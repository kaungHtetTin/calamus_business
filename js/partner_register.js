  ...
  fetch("api/register.php?endpoint=check_email", {
    ...
    .then((data) => {
      ...
      if (statusDiv && data.success) {
        if (data.available) {
          statusDiv.innerHTML =
            '<i class="fas fa-check text-success"></i> Email is available';
          statusDiv.className = "form-text text-success";
        } else {
          statusDiv.innerHTML =
            '<i class="fas fa-times text-danger"></i> Email already registered';
    ...
    fetch("api/register.php?endpoint=register", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          showAlert(
            `Registration successful! Your private code is: <strong>${privateCode}</strong><br>Please save this code and check your email for verification instructions.`,
            "success"
          );
